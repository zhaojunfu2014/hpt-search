...
public class FileUtil {
    ...
    private static boolean copyFile(String srcPath, String destDir) {  
        ...
        String fileName = srcPath  
                .substring(srcPath.lastIndexOf(separator));  
        String destPath = destDir + fileName;  
        if (destPath.equals(srcPath)) { // 源文件路径和目标文件路径重复  
            log.debug("源文件路径和目标文件路径重复!");  
            return false;  
        }  
        File destFile = new File(destPath);  
        if (destFile.exists() && destFile.isFile()) { // 该路径下已经有一个同名文件  
            log.debug("目标目录下已有同名文件!");  
            return false;  
        }  
    ...
    private static boolean copyDirectory(String srcPath, String destDir) {  
        ...
        // 目标文件夹的完整路径  
        String destPath = destDir + separator + dirName;  
        // log.debug("目标文件夹的完整路径为：" + destPath);  
  
        if (destPath.equals(srcPath)) {  
            log.debug("目标文件夹与源文件夹重复");  
            return false;  
        }  
        File destDirFile = new File(destPath);  
        if (destDirFile.exists()) { // 目标位置有一个同名文件夹  
            log.debug("目标位置已有同名文件夹!");  
            return false;  
        }  
    ...
    private static String getDirName(String dir) {  
        if (dir.endsWith(separator)) { // 如果文件夹路径以"//"结尾，则先去除末尾的"//"  
    ...
    public static boolean cutGeneralFile(String srcPath, String destDir) {  
        if (!copyGeneralFile(srcPath, destDir)) {  
            log.debug("复制失败导致剪切失败!");  
            return false;  
        }  
        if (!deleteGeneralFile(srcPath)) {  
            log.debug("删除源文件(文件夹)失败导致剪切失败!");  
            return false;  
        }  
